...
def predict_user(user1_name, user2_name, tweet_text, cache=None):
    """determine which user more likely to tweet the text"""
    ...
    return (log_reg.predict(np.array(tweet_embedding).reshape(1, -1)), log_reg.predict_proba(np.array(tweet_embedding).reshape(1, -1)))
